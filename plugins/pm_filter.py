...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("Vous utilisez un de mes anciens messages, veuillez renvoyer la demande.", show_alert=True)
        return
    ...
    btn.insert(0,
        [
            InlineKeyboardButton(text="⚡ʜᴏᴡ ᴛᴏ ᴅᴏᴡɴʟᴏᴀᴅ⚡", url='https://t.me/trailer_cinemalakay')
        ]
    )
...
async def advantage_spoll_choker(bot, query):
    ...
    if int(user) != 0 and query.from_user.id != int(user):
        return await query.answer("😁 𝗛𝗲𝘆 𝗙𝗿𝗶𝗲𝗻𝗱,Effectuez votre propre requête.", show_alert=True)
    if movie_ == "close_spellcheck":
        return await query.message.delete()
    movies = SPELL_CHECK.get(query.message.reply_to_message.id)
    if not movies:
        return await query.answer("Lien expiré Veuillez rechercher à nouveau 🙂.", show_alert=True)
    ...
    if k == False:
        ...
        if files:
            ...
            k = await query.message.edit('CE FILM N'EST PAS ENCORE SORTI OU AJOUTÉ À LA BASE DE DONNÉES 💌')
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("J'aime ton intelligence, mais ne sois pas trop intelligent Okay 😒", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
        ...
        buttons = [[
            ...
            InlineKeyboardButton('⚡ Les Nouvelles ⚡', url=f"https://t.me/+HZ5Y0DGqJ3hlZWJh"),
            InlineKeyboardButton('🧑 Popriétaire 🧑', url='https://t.me/Contact_Star_Plus_Bot')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if imdb:
        ...
        cap = f"Rᴇǫᴜᴇsᴛᴇᴅ ᴍᴏᴠɪᴇ ɴᴀᴍᴇ : <code>{search}</code>\n\n\n😌 SI LE FILM QUE VOUS RECHERCHEZ N'EST PAS DISPONIBLE, LAISSEZ UN MESSAGE CI-DESSOUS 😌 \n\nᴇxᴀᴍᴘʟᴇ : \n\nENTREZ LE NOM DE VOTRE FILM (ANNÉE) ᴛᴀɢ @admin"
